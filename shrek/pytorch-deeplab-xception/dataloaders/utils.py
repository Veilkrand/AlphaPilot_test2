...
def cross_entropy2d(logit, target, ignore_index=255, weight=None, size_average=False, batch_average=True):
    n, c, h, w = logit.size()
...
def get_falseneg_falsepos(pred, gt, n_classes=21):
    ...
    for i in range(len(pred)):
        ...
        for j in range(n_classes):
            match = (pred_tmp == j) + (gt_tmp == j)
            match_false_negative = (gt_tmp == j) - (pred_tmp == j)
            match_false_positive = (pred_tmp == j) - (gt_tmp == j)

            it = torch.sum(match == 2).item()
            un = torch.sum(match > 0).item()
            fn = torch.sum(match_false_negative == 1).item()
            fp = torch.sum(match_false_positive == 1).item()
        ...
        for k in range(n_classes):
            if union[k] == 0:
                continue
            fn_iou.append(false_negative[k] / union[k])
            fp_iou.append(false_positive[k] / union[k])

        img_fn_iou = (sum(fn_iou) / len(fn_iou))
        total_false_negative += img_fn_iou

        img_fp_iou = (sum(fp_iou) / len(fp_iou))
        total_false_positive += img_fp_iou

    return total_false_negative, total_false_positive
