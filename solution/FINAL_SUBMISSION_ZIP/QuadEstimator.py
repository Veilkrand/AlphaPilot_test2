...
class QuadEstimator():
	...
	def _find_shapes(self, img_bw, img_original, gray=False, debug=True):

		if gray is True:
			img_result = cv2.cvtColor(img_original, cv2.COLOR_GRAY2RGB)
		else:
	...
	def _get_inner_area_corners_from_results(self, results):
		...
		if len(results) == 1:
			...
			print('** Two candidates. Selecting the smaller area')
			inner_shape = None
		...
		if inner_shape is not None and 'corners' in inner_shape and len(inner_shape['corners'])==4:
			...
			# TODO: handle more than two candidates by common centroid grouping
			return None

	def process_img(self, img_original, gray=False):
		img_bw = self._preprocess_img(img_original, gray)

		img_shapes, result = self._find_shapes(img_bw, img_original, gray)
